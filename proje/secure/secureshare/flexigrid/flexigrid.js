...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 rpOptions: [10,15,20,25,40,100,500,1000,2000],
			 title: true,
			 // pagestat: 'Listelenen : {from} - {to}. Toplam : {total}',
			 pagestat: 'Toplam veri sayısı : {total}',
			 procmsg: 'Yükleniyor, lütfen bekleyin...',
			 query: '',
			 qtype: '',
			 nomsg: 'Veri bulunamadı',
			...
			populate: function () { //get latest data
				...
				if (p.params)
					...
					$.ajax({
					   ...
					   dataType: 'json',
					   success: function(data){ g.addData(data);  },
					   error: function(data, textStatus, errorThrown) { alert(textStatus +' | ' +errorThrown + ' | ' + data.responseText); }
							  });		   					   
					   /* Shopphp edit  */
		...
		$(g.bDiv).after(g.pDiv);
		var html = ' <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">Sayfa <input type="text" size="4" value="1" /> Toplam : <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
		...
		if (p.searchitems)
			{
				...
				$(g.sDiv).append("<div class='sDiv2'>Arama <input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>"+sopt+"</select> <input type='button' value='Temizle' /></div>");

				$('input[name=q],select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13) g.doSearch()});
				$('input[value=Temizle]',g.sDiv).click(function(){$('input[name=q]',g.sDiv).val(''); p.query = ''; g.doSearch(); });
				$(g.bDiv).after(g.sDiv);
				$(g.sDiv).show();
		...
		if ($('th',g.hDiv).length)
		{
			...
			.html('<div></div>')
			.attr('title','Sütunu Gizle/Göster')
			.click
			(
			 	function ()
				{
