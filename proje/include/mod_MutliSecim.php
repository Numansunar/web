...
if ($_POST['mod_MS_ajaxUpload']) {
	...
	if ($_SESSION['upload_' . $_POST['mod_MS_urunID']]) {
		...
		$filename = $_SESSION['randStr'] . '-' . ($_POST['mod_MS_urunID']) . '-' . $_POST['mod_MS_ajaxFileNum'] . '.jpg';
...
function mod_MS_selectAdet($d)
{
	...
	$i = 1;
	$out .= '<li class="sf-form-item-fullwidth"><div><label class="sf-text-label">Paket Seçimi</label><select class="secim secim" name="paket" id="paket-adet"><option value="">' . _lang_form_lutfenSecin . '</option>';
	foreach ($arr as $a) {
		if (!$a)
			continue;
		...
		$out .= '<option value="' . $adet . '">' . $a . ' ('.$adet.' ad.)</option>' . "\r\n";
		$i++;
	}
	$out .= '</select></div></li>';
	$out .= '</ul></fieldset>';
	$out .= "<script type='text/javascript'>
		$('#paket-adet').change(function() { urunSepeteEkleAdet = $(this).val(); $('.urunSepeteEkleAdet').val($(this).val()); ajaxFiyatGuncelle(".$d['ID'].");  });
	</script>";
	return $out;
}
...
{
	$out .= '<script src="js/jquery.form.js"></script>
