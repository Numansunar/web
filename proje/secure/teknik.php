...
function updateUrunResim($url, $ID, $field, $name = '')
{
	...
	return adminInfov3('<strong>' . $name . ' (' . $ID . ')</strong> ürün ' . $field . ' <strong>' . $resimLocation . '</strong> (' . $url . ') ile güncellendi.');
}
...
if ($shopphp_demo && $_GET['act']) {
	...
	echo v4Admin::fullBlock('Teknik İşlemler', '<div style="padding:15px;">Güvenlik nedeniyle teknik işlemler devredışıdır.</div>');
} else {
	$tempInfo = adminWarnv3('Teknik işlemlerden yapılan işlemler geri alınamaz. Emin olmadığınız işlemlerde <strong>mutlaka site ve veritabanı yedeğini alın</strong>.');
	switch ($_GET['act']) {
		...
		case 'breadcrumb':
			...
			$tempInfo .= adminInfov3("Toplam <strong>$urun</strong> ürün ve <strong>$cat</strong> kategori güncellendi.");
			if ($_GET['act'] == 'remove-empty-cats') {
				$q = my_mysql_query("select * from kategori where active = 1");
				while ($d = my_mysql_fetch_array($q)) {
					if (!hq("select ID from urun where catID='" . $d['ID'] . "' OR showCatIDs like '%|" . $d['ID'] . "|%'"))
		...
		case 'repairdb':
			...
			while ($arr2 = my_mysql_fetch_array($rs2)) {
				...
				$tempInfo .= adminInfov3('<strong>' . $arr2[0] . '</strong> tablosuna tamir konutu gönderildi. <strong>(' . $d['Msg_text'] . ')</strong>');
			}
			break;
		case 'repair-basket':
			...
			$tempInfo .= adminInfov3('Sepet sipariş durum değerleri güncellendi.');
			break;
		case 'optimizedb':
			$rs2 = my_mysql_query("show tables");
			while ($arr2 = my_mysql_fetch_array($rs2)) {
				...
				$d = my_mysql_fetch_array($q);
				$tempInfo .= adminInfov3('<strong>' . $arr2[0] . '</strong> tablosuna optimize konutu gönderildi. <strong>(' . $d['Msg_text'] . ')</strong>');
			}
			break;
		case 'cleanbrand':
			$mi = 0;
			...
			while ($d = my_mysql_fetch_array($q)) {
				...
				//$tempInfo.=adminInfov3($markaName.' Kontrol.');
				while ($d2 = my_mysql_fetch_array($q2)) {
					...
					$tempInfo .= adminInfov3('<strong>' . $d2['name'] . ' (' . $d2['ID'] . ')</strong> markasından birden fazla olduğundan silindi.');
					$mi++;
				}
			}
			$tempInfo .= adminInfov3('Toplam <strong>' . $mi . '</strong> marka ismi benzer olduğu için silindi ve ürünleri güncellendi.');
			break;
		case 'fixgtin':
			$mi = 0;
			...
			$tempInfo .= adminInfov3('Toplam <strong>' . $mi . '</strong> ürün / varyasyon GTIN değeri güncellendi.');
			break;
		case 'fixbrand':
			...
			$tempInfo .= adminInfov3('Toplam <strong>' . $mi . '</strong> ürün marka seçilmediği için güncellendi.');
			break;
		case 'updateproductIDs':
			...
			$tempInfo .= adminInfov3('Toplam <strong>' . $id . '</strong> ürün ID güncellendi.');
			break;
		case 'updateproductnames':
			...
			$tempInfo .= adminInfov3('Toplam <strong>' . $id . '</strong> ürün adı güncellendi.');
			break;
		...
		case 'cleanproduct':
			...
			while ($d = my_mysql_fetch_array($q)) {
				...
				while ($d2 = my_mysql_fetch_array($q2)) {
					...
					$tempInfo .= adminInfov3('<strong>' . $d2['name'] . ' (' . $d2['ID'] . ')</strong> ürününden birden fazla olduğundan, pahalı olan ürün pasif yapıldı.');
					$mi++;
				}
			}
			$tempInfo .= adminInfov3('Toplam <strong>' . $mi . '</strong> ürün, ismi benzer olduğu için pasif edildi.');
			break;
		case 'cleancat':
			$mi = 0;
			...
			while ($d = my_mysql_fetch_array($q)) {
				...
				while ($d2 = my_mysql_fetch_array($q2)) {
					...
					$tempInfo .= adminInfov3('<strong>' . $d2['name'] . ' (' . $d2['ID'] . ')</strong> kategorisinden birden fazla olduğundan silindi.');
					$mi++;
				}
			}
			$tempInfo .= adminInfov3('Toplam <strong>' . $mi . '</strong> kategori ismi benzer olduğu için silindi ve ürünleri güncellendi.');
			break;
		...
		case 'updatev3stocks':
			...
			$tempInfo .= adminInfov3('Toplam <strong>' . $mi . '</strong> ürün varyasyon kontrolü yapıldı ve güncellendi.');
			break;
		...
		case 'updatevarstocks':
			...
			while ($d = my_mysql_fetch_array($q)) {
				...
				if (!hq("select ID from urunvarstok where urunID='" . $d['urunID'] . "' AND (var1 like '" . $name . "' OR var2 like'" . $name . "') ")) {
					...
					my_mysql_query("insert into urunvarstok (ID,urunID,var1,fark,stok) VALUES (null,'" . $d['urunID'] . "','$name','$fark','" . $stok . "')");
			...
			$tempInfo .= adminInfov3('Toplam <strong>' . $mi . '</strong> ürün varyasyon kontrolü yapıldı ve <strong>' . $mx . '</strong> kayıt güncellendi.');
			break;
		...
		case 'updatestocks':
			...
			while ($d = my_mysql_fetch_array($q)) {
				...
				if ($d['ozellik1'] && $d['ozellik1detay']) {
					$xarr = explode("\n", $d['ozellik1detay']);
					foreach ($xarr as $x) {
						list($a, $b, $c) = explode('|', $x);
						$v1 = array($d['ozellik1'], $a, $b);
						if ($d['ozellik1'])
							urunVarKontrol($v1, '', $c, '', $urunID);
					}
				}

				if ($d['ozellik2'] && $d['ozellik2detay']) {
					$xarr = explode("\n", $d['ozellik2detay']);
					foreach ($xarr as $x) {
						list($a, $b, $c) = explode('|', $x);
						$v2 = array($d['ozellik2'], $a, $b);
						if ($d['ozellik2'])
							urunVarKontrol('', $v2, $c, '', $urunID);
					}
				}
			...
			$tempInfo .= adminInfov3('Toplam <strong>' . $mi . '</strong> ürün stok sayısı, varyasyon stok sayısı ile eşitlendi.');
			break;
		case 'resizepics':
				...
				for ($i = 1; $i <= 10; $i++) {
					if ($d['resim' . ($i > 1 ? $i : '')]) {
						if (is_array(getimagesize('../images/urunler/' . $d['resim' . ($i > 1 ? $i : '')]))) {
							resize(1000, '../images/urunler/' . $d['resim' . ($i > 1 ? $i : '')], '../images/urunler/' . $d['resim' . ($i > 1 ? $i : '')]);
							$tempInfo .= adminInfov3('<strong>' . $d['name'] . ' (' . $d['ID'] . ')</strong> ait resim' . ($i > 1 ? $i : '') . ' yeniden boyutlandırıldı.');
							$mi++;
						} else
							$tempInfo .= adminWarnv3('<strong>' . $d['name'] . ' (' . $d['ID'] . ')</strong> ait resim' . ($i > 1 ? $i : '') . ' geçersiz algılandı. Bu paneldenki "Hatalı resimleri sil" seçeneği ile bu resimleri toplu silebilirsiniz.');
					}
				}
			}
			$tempInfo .= adminInfov3('Toplam <strong>' . $mi . '</strong> ürün resmi yeniden boyutlandırıldı.');
			break;
		case 'allpics':
			$ri = 0;
			$ci = 0;
			foreach (glob('../images/urunler/*.*') as $v) {
				...
				{
					if (getimagesize($v)) {
						$tempInfo .= adminInfov3('<strong>' . $v . '</strong> isimli resim  silindi.');
						$ri++;
						unlink('../images/urunler/'.$v);
					} else {
						$tempInfo .= adminInfov3('<strong>' . $v . '</strong> dosyası bir resim değil. Üzerinde işlem yapılmadı.');
					}
				}
				$ci++;
			}
			$tempInfo .= adminInfov3('Toplam <strong>' . $ci . '</strong> dosya kontrol edildi. <strong>' . $ri . '</strong> resim için silindi.');
			break;
		case 'brokenpics':
			$mi = 0;
			$q = my_mysql_query("select * from urun where resim != ''");
			while ($d = my_mysql_fetch_array($q)) {
				for ($i = 1; $i <= 10; $i++) {
					if ($d['resim' . ($i > 1 ? $i : '')] && (!file_exists('../images/urunler/' . $d['resim' . ($i > 1 ? $i : '')]) || !getimagesize('../images/urunler/' . $d['resim' . ($i > 1 ? $i : '')]))) {
						...
						$tempInfo .= adminInfov3('<strong>' . $d['name'] . ' (' . $d['ID'] . ')</strong> ait resim' . ($i > 1 ? $i : '') . ' bulunamadığından veya hatalı olduğundan field boşaltıldı.');
						if (!getimagesize('../images/urunler/' . $d['resim' . ($i > 1 ? $i : '')])) {
							unlink('../images/urunler/' . $d['resim' . ($i > 1 ? $i : '')]);
							$tempInfo .= adminInfov3('<strong>' . $d['name'] . ' (' . $d['ID'] . ')</strong> ait resim' . ($i > 1 ? $i : '') . ' hatalı olduğundan silindi.');
						}
						$mi++;
					}
				}
			}
			$tempInfo .= adminInfov3('Toplam <strong>' . $mi . '</strong> ürün resmi bulunamadığı için, resim field\'ı boşaltıldı.');
			break;
		case 'seopics':
			$mi = 0;
			...
			while ($d = my_mysql_fetch_array($q)) {
				for ($i = 1; $i <= 5; $i++) {
					...
					if (my_mysql_affected_rows()) {
						...
						$tempInfo .= adminInfov3('<strong>' . $d['name'] . ' (' . $d['ID'] . ')</strong> ait ' . $field . ' SEO ya uygun olarak "' . $filename . '" dosya adıyla güncellendi.');
						$mi++;
					}
				}
			}
			$tempInfo .= adminInfov3('Toplam <strong>' . $mi . '</strong> ürün resmi SEO uygun bir şekilde güncellendi.');
			break;
		case 'passivepics':
			$ri = 0;
			$ci = 0;
			my_mysql_query("update urun set active = 0 where resim = ''");
			$tempInfo .= adminInfov3('Toplam <strong>' . my_mysql_affected_rows() . '</strong> ürün, resmi olmadığı için pasif edildi.');
			break;
		case 'cleanpics':
			$ri = 0;
			$ci = 0;
			foreach (glob('../images/urunler/*.*') as $v) {
				...
				if (!hq("select ID from urun where resim like '$r' OR resim2 like '$r' OR resim3 like '$r' OR resim4 like '$r' OR resim5 like '$r'")) {
					if (getimagesize($v)) {
						$tempInfo .= adminInfov3('<strong>' . $v . '</strong> isimli resim kullanılmadığından silindi.');
						$ri++;
						unlink($v);
					} else {
						$tempInfo .= adminInfov3('<strong>' . $v . '</strong> dosyası bir resim değil. Üzerinde işlem yapılmadı.');
			...
			$tempInfo .= adminInfov3('Toplam <strong>' . $ci . '</strong> dosya kontrol edildi. <strong>' . $ri . '</strong> resim kullanılmadığı için silindi.');
			break;
		case 'cleanimagecache':
			delTree('../images/cache/');
			delTree('images/cache/');
			$tempInfo .= adminInfov3('images/cache dizini boşaltıldı.');
			break;
		case 'emaillogclean':
			my_mysql_query("delete from epostalog");
			$tempInfo .= adminInfov3('E-Posta logları silindi.');
			break;
		case 'query':
			if ($_POST['query']) {
				$_POST['query'] = stripslashes($_POST['query']);
				if (!$_SESSION['admin_isAdmin'] || $shopphp_demo)
					$tempInfo .= adminInfov3('Bu işlem sadece admin yetkisindeki kullanıcılar tarafından kullanılabilir.');
				else {
					require_once('../doc/import-lib.php');
					if (stristr($_POST['query'], ';') === false)
						$_POST['query'] .= ';';
					mysql_import_text($_POST['query']);
					$tempInfo .= adminInfov3('<pre>' . $_POST['query'] . '</pre>Veritabanı sorgusu çalıştırıldı.');
				}
			}
			break;
		case 'utf8-bom':
					...
					if ($file->isFile()) {
						...
						if (isset($path_parts['extension']) && in_array($path_parts['extension'], $check_extensions)) {
							...
							if (false !== strpos($line, STR_BOM)) {
								$tempInfo .= adminInfov3($file->getRealPath().' Dosyasında UTF8 BOM başlangıcı var.');
							}
							else if (substr($line,0,1) != '<') {
								$tempInfo .= adminInfov3($file->getRealPath().' Dosyasında UTF8 BOM yok fakat PHP tag öncesi bir karakter var.');
							}
						}
					}
				}
			...
			$tempInfo .= adminInfov3('UTF-8 BOM bulunan dosyaların bir editör ile açılıp, BOM Singature \'ın kaldırılması gerekmektedir.');
			break;
		case 'temp-2-utf8':
			...
			$tempInfo .= adminInfov3('Toplam <strong>' . (int) $editUTF8OK . '</strong> dosya encode UTF-8 olarak güncellendi. Kontrol edilen <strong>' . (int) $editUTF8Skip . '</strong> dosya zaten UTF-8 olduğundan güncellenmedi.');
			break;
		case 'repair-seo':
			...
			$tempInfo .= adminInfov3('Ürun, kategori, marka, makale, makalekategori,haberler ve pages tablolarında SEO fieldları düzenlendi.');
			break;
		case 'update-lang':
			updateTLang();
			break;
		case 'updateuserorder':
			...
			while ($d = my_mysql_fetch_array($q)) {
				...
				$tempInfo .= adminInfov3('Kullanıcı : ' . $d['name'] . ' ' . $d['lastname'] . ' cirosu güncellendi.');
			}
			break;
		case 'var-filter':
			...
			while ($d = my_mysql_fetch_array($q)) {
				...
				$tempInfo .= adminInfov3('Ürün ' . $d['name'] . ' (' . $d['urunID'] . ') için <strong>' . $title . '</strong> özelliğinde ve <strong>' . $d['var'] . '</strong> değerinde filtre kontrolü yapıldı.');
			}
			break;
		case 'cronjoblogview':
			$tempInfo .= adminInfov3('
		...
		case 'emptycats':
			...
			$tempInfo .= adminInfov3('Toplam ' . (int) my_mysql_num_rows($q) . ' kategori kontrol edilecek.');
			while ($d = my_mysql_fetch_array($q)) {
				if (!hq("select ID from urun where active = 1 AND (urun.catID='" . $d['ID'] . "' OR urun.virtualCatIDs like '%," . $d['ID'] . ",%' OR showCatIDs like '%|" . $d['ID'] . "|%' or kategori.idPath like '" . $d['idPath'] . "/%') limit 0,1")) {
					...
					$tempInfo .= adminInfov3('Kategori ' . $d['name'] . ' (' . $d['ID'] . ') içinde aktif ürün bulunmadığından pasif edildi.');
				} else 
						if (!$d['active']) {
					my_mysql_query("update kategori set active = 1 where ID='" . $d['ID'] . "'");
					$tempInfo .= adminInfov3('Kategori ' . $d['name'] . ' (' . $d['ID'] . ') içinde aktif ürün bulunduğundan aktif edildi.');
				}
			}
		...
		case 'update':
			switch (siteConfig('version')) {
				...
				case '3.5':
					...
					break;
				case '3.6':
					$gArr = array('v36_to_v37_sql.txt', 'v37_to_v38_sql.txt', 'v38_to_v39_sql.txt', 'v39_to_v395_sql.txt', 'v395_to_v4_sql.txt', 'v40_to_v41_sql.txt', 'v41_to_v42_sql.txt', 'v42_to_v43_sql.txt', 'v43_to_v44_sql.txt', 'v44_to_v45_sql.txt', 'v45_to_v46_sql.txt', 'v46_to_v47_sql.txt', 'v47_to_v48_sql.txt', 'v48_to_v49_sql.txt','v49_to_v495_sql.txt');
					break;
				case '3.7':
					$gArr = array('v37_to_v38_sql.txt', 'v38_to_v39_sql.txt', 'v39_to_v395_sql.txt', 'v395_to_v4_sql.txt', 'v40_to_v41_sql.txt', 'v41_to_v42_sql.txt', 'v42_to_v43_sql.txt', 'v43_to_v44_sql.txt', 'v44_to_v45_sql.txt', 'v45_to_v46_sql.txt', 'v46_to_v47_sql.txt', 'v47_to_v48_sql.txt', 'v48_to_v49_sql.txt','v49_to_v495_sql.txt');
					break;
				case '3.8':
					$gArr = array('v38_to_v39_sql.txt', 'v39_to_v395_sql.txt', 'v395_to_v4_sql.txt', 'v40_to_v41_sql.txt', 'v41_to_v42_sql.txt', 'v42_to_v43_sql.txt', 'v43_to_v44_sql.txt', 'v44_to_v45_sql.txt', 'v45_to_v46_sql.txt', 'v46_to_v47_sql.txt', 'v47_to_v48_sql.txt', 'v48_to_v49_sql.txt','v49_to_v495_sql.txt');
					break;
				case '3.9':
					$gArr = array('v39_to_v395_sql.txt', 'v395_to_v4_sql.txt', 'v40_to_v41_sql.txt', 'v41_to_v42_sql.txt', 'v42_to_v43_sql.txt', 'v43_to_v44_sql.txt', 'v44_to_v45_sql.txt', 'v45_to_v46_sql.txt', 'v46_to_v47_sql.txt', 'v47_to_v48_sql.txt', 'v48_to_v49_sql.txt','v49_to_v495_sql.txt');
					break;
				case '3.95':
					$gArr = array('v395_to_v4_sql.txt', 'v40_to_v41_sql.txt', 'v41_to_v42_sql.txt', 'v42_to_v43_sql.txt', 'v43_to_v44_sql.txt', 'v44_to_v45_sql.txt', 'v45_to_v46_sql.txt', 'v46_to_v47_sql.txt', 'v47_to_v48_sql.txt', 'v48_to_v49_sql.txt','v49_to_v495_sql.txt');
					break;
				case '4.0':
					$gArr = array('v40_to_v41_sql.txt', 'v41_to_v42_sql.txt', 'v42_to_v43_sql.txt', 'v43_to_v44_sql.txt', 'v44_to_v45_sql.txt', 'v45_to_v46_sql.txt', 'v46_to_v47_sql.txt', 'v47_to_v48_sql.txt', 'v48_to_v49_sql.txt','v49_to_v495_sql.txt');
					break;
				case '4.1':
					$gArr = array('v41_to_v42_sql.txt', 'v42_to_v43_sql.txt', 'v43_to_v44_sql.txt', 'v44_to_v45_sql.txt', 'v45_to_v46_sql.txt', 'v46_to_v47_sql.txt', 'v47_to_v48_sql.txt', 'v48_to_v49_sql.txt','v49_to_v495_sql.txt');
					break;
				case '4.2':
					$gArr = array('v42_to_v43_sql.txt', 'v43_to_v44_sql.txt', 'v44_to_v45_sql.txt', 'v45_to_v46_sql.txt', 'v46_to_v47_sql.txt', 'v47_to_v48_sql.txt', 'v48_to_v49_sql.txt','v49_to_v495_sql.txt');
					break;
				case '4.3':
					$gArr = array('v43_to_v44_sql.txt', 'v44_to_v45_sql.txt', 'v45_to_v46_sql.txt', 'v46_to_v47_sql.txt', 'v47_to_v48_sql.txt', 'v48_to_v49_sql.txt','v49_to_v495_sql.txt');
					break;
				case '4.4':
					$gArr = array('v44_to_v45_sql.txt', 'v45_to_v46_sql.txt', 'v46_to_v47_sql.txt', 'v47_to_v48_sql.txt', 'v48_to_v49_sql.txt','v49_to_v495_sql.txt');
					break;
				case '4.5':
					$gArr = array('v45_to_v46_sql.txt', 'v46_to_v47_sql.txt', 'v47_to_v48_sql.txt', 'v48_to_v49_sql.txt','v49_to_v495_sql.txt');
					break;
				case '4.6':
					$gArr = array('v46_to_v47_sql.txt', 'v47_to_v48_sql.txt', 'v48_to_v49_sql.txt','v49_to_v495_sql.txt');
					break;
				case '4.7':
					$gArr = array('v47_to_v48_sql.txt', 'v48_to_v49_sql.txt','v49_to_v495_sql.txt');
					break;
				case '4.8':
					$gArr = array('v48_to_v49_sql.txt','v49_to_v495_sql.txt');
					break;
				case '4.9':  
					$gArr = array('v49_to_v495_sql.txt');
					break;
				case '4.95':
					$tempInfo .= adminInfov3('Yazılım sürümünüz güncel. <strong>(v' . siteConfig('version') . ')</strong>');
					break;
				default:
					$tempInfo .= adminErrorv3('Desteklenmeyen yazılım sürümü (v'.siteConfig('version').'). Otomatik güncelleme modülü için, bir önceki sürümünüzün v3.0 ve üzeri olması gerekmektedir.<br><a href="http://yardim.shopphp.net/page.php?act=kategoriGoster&katID=322">Sorguları manuel çalıştırmak için bu siteyi ziyaret edin.</a>.');
					break;
			}
			...
			if (is_array($gArr)) {
				...
				foreach ($gArr as $file) {
					...
					if (mysql_import('../doc/sql/' . $file)) {
						$tempInfo .= adminInfov3($file . ' sorgusu başarıyla çalıştırıldı.');
						$gi++;
					}
				}
				$tempInfo .= adminInfov3('Toplam ' . $gi . ' sorgu başarıyla çalıştırıldı.');
				...
				$tempInfo .= adminInfov3('Toplam <strong>' . (int) $editUTF8OK . '</strong> dosya encode UTF-8 olarak güncellendi. Kontrol edilen <strong>' . (int) $editUTF8Skip . '</strong> dosya zaten UTF-8 olduğundan güncellenmedi.');
				//	$tempInfo.=adminInfov3('Güncel dil dosyası kontrolü için <a href="s.php?f=teknik.php&act=update-lang"><strong>tıklayın.</strong></a>');
				$tempInfo .= adminInfov3('Şablon dizinideki pasif olan diğer şablonların UTF-8 encode kontrolü için <a href="s.php?f=teknik.php&act=temp-2-utf8"><strong>tıklayın.</strong></a>');
			}
	...
	echo v4Admin::adminHeader();
	if ($_GET['act'] == 'query') {
		echo v4Admin::fullBlock('Veritabanı Sorgusu', sqlQueryWindow()) . '<div class="clear-space">&nbsp;</div>';
		unset($tempInfo);
	}
	if ($_GET['act'] == 'updatepics-google' || $_GET['act'] == 'updatepics-bing') {
		echo v4Admin::fullBlock('Resim Güncelle', noPicCatList()) . '<div class="clear-space">&nbsp;</div>';
		unset($tempInfo);
	}
	echo v4Admin::fullBlock('Teknik İşlemler', teknik());
	if ($_GET['act'])
		cleancache();
}
...
function noPicCatList()
{
	...
	$out = '	<form method="post" action="" class="form-horizontal form-bordered">
			' . v4Admin::simpleFormGroup('Resimsiz Ürün Kategorileri', $pout . ' ' . $select) . '			
			' . v4Admin::simpleFormFooter('Resimleri Güncelle') . '

	</form>';
	$tempInfo .= adminInfov3('Resim olmayan <strong>' . (int) $totals . '</strong> ürün bulundu.');
	return $out;
}
...
function teknik()
{
	global $tempInfo;
	$tArray = array(
		'query' => 'Veritabanı sorgusu çalıştır',
		...
		'updateproductnames' => 'Ürün Başlıklarını Düzenle',
		'updateproductprices' => 'Ürün Fiyatlarını Yuvarla',
		...
		'resizepics' => 'Ürün kaynak resimlerini web için boyutlandır (1000px)',
	...
	$out = '<div class="button-box"><table class="teknik-table"><tr><td><select id="teknik"><option value="">Lütfen işlemi seçin </option>';
...
function updateTLang()
{
	...
	global $tempInfo;
	$url = 'http://shopphpv4.shopphp.net/langxml.php';
	$ch = curl_init();
	...
	foreach ($open->dil as $dil) {
		...
		if (!hq("select ID from lang where `key` like '" . $dil->xkey . "'")) {
			...
			$tempInfo .= adminInfov3('Dil veri tabanında ' . $dil->xkey . ' olmadığından <xmp>' . $dil->xvalue . '</xmp> değeri ile eklendi.');
			$dili++;
		}
	}
	if ($dili)
		$tempInfo .= adminInfov3('Dil güncelleme kontrolü tamamlandı. Veritabanında bulunmayan toplam ' . $dili . ' satır eklendi.');
	else
		$tempInfo .= adminInfov3('Dil güncelleme kontrolü tamamlandı. Veritabanında eksik dil satırı bulunmuyor.');
}
