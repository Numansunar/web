...
<? echo '<base href="http'.($siteConfig['httpsAktif'] && ($_SERVER[HTTPS] && $_SERVER[HTTPS] != 'off')?'s':'').'://'.$_SERVER['HTTP_HOST'].$siteDizini.'">'; ?>

<title><?=$siteConfig['title'];?></title>
<link rel="stylesheet" href="css/popup.compare.css" />
...
$infoArray = array(_lang_compareName=>'name',_lang_compareOnDetay=>'onDetay',_lang_compareFiyat=>'fiyat',_lang_compareMarka=>'markaID',_lang_compareKDV=>'kdv',_lang_compareGaranti=>'garanti',_lang_compareTarih=>'tarih',_lang_compareStok=>'stok',_lang_comparePuan=>'puan',_filtre=>'_filtre',_lang_compareOzellikler=>'detay');
##############################################
			...
			switch ($v) {
				...
				case 'fiyat':
					if($_SESSION['userID'] || !siteConfig('fiyatUyelikZorunlu'))
						$urun[$j][$v] = my_money_format('%i',kdvHaricFiyat($urun[$j])).' '.$urun[$j]['fiyatBirim'].' + KDV ('.my_money_format('%i',YTLfiyat(fixFiyat($urun[$j][$v]),$urun[$j]['fiyatBirim'])).' TL)';
					else
				...
				case 'puan':
					...
					$urun[$j][$v] = '<img src="templates/'.$siteConfig['templateName'].'/images/stars_'.$ortalamaPuan.'.png">';
				break;
			}	
...
echo '</table>';
if(!$_GET['act'])
	echo "<br><input type='button' value='"._lang_listeyiTemizle."' onclick=\"window.location.href = 'compare.php?KarsilastirmaListeTemizle=true'\">";
//else
