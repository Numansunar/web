...
function pcToplama() {
	...
	while($d=my_mysql_fetch_array($q)) {
		...
		$out.='<tr class="catTr"><td class="catName"><nobr>'.$d['name'].' : </nobr></td><td id="catNum_'.$i.'"><select onchange="'.$onChangeInsert.'updateFiyat(this.options[this.selectedIndex].value,\''.$d['ID'].'\')" id="urunSelected_'.$d['ID'].'" name="pctURUN[]"><option value=0>Lütfen Seçin</option>';
	...
	$out.='</select> <span id="pcToplamaSatirEkle">satırı ekle</span></td></tr>';
	$out.='<tr><td colspan=6>';
	$out.='<div class="sepetToplam">';
		$out.='<table>';
		$out.='<tr><td class="td1">Sistem Toplamı (KDV Dahil)</td><td class="td2">:</td><td class="td3"><span ID="kdvdahil">0</span> TL</td></tr>';
