  ...
  function getHtml()
  {
    ...
    if (SpawAgent::getAgent() != SPAW_AGENT_UNSUPPORTED) {
      ...
      foreach ($this->pages as $pname => $page) {
        $pagetpl .= '<textarea name="' . $page->inputName . '" id="' . $pname . '" style="width: 100%; height: ' . $this->height . '; display: none; overflow: scroll;" rows="10" cols="10">' . htmlspecialchars($page->value, ENT_COMPAT, 'ISO-8859-1', true) . '</textarea>';
        $js_res .= 'var ' . $pname . '_page = new SpawEditorPage("' . $pname . '","' . htmlspecialchars($page->caption, ENT_COMPAT, 'ISO-8859-1', true) . '","' . $page->direction . '");' . "\n";
        //$js_res .= $pname.'_page.value = "'.htmlspecialchars($page->value, ENT_COMPAT,'ISO-8859-1', true).'";'."\n";
      ...
      foreach ($this->pages as $pname => $page) {
        ...
        $res .= '<textarea name="' . $pname . '" id="' . $pname . '" width="' . $this->width . '" height="' . $this->height . '" style="width: ' . $this->width . '; height: ' . $this->height . ';" wrap="off">' . htmlspecialchars($page->value, ENT_COMPAT, 'ISO-8859-1', true) . '</textarea><br />';
      }
    }
    return $res;
  }
...
function dbekle($dbase, $icerik, $ozellikler, $id)
{
  if ($ozellikler['insertDisabled']) return;
  global $baglanti, $uploadAllowed, $showSPError;
  $dosyalar = array();
  ...
  for ($i = 0; $i < sizeof($icerik); $i++) {
    ...
    switch ($icerik[$i][stil]) {
      case "date":
        $_POST[$db] = $_POST[$db . '_yil'] . '-' . $_POST[$db . '_ay'] . '-' . $_POST[$db . '_gun'] . ' ' . $_POST[$db . '_saat'] . ':' . $_POST[$db . '_dak'] . ':00';
        break;
      case "checkbox":
      ...
      case "multiplechoice":
        if (sizeof($_POST[$db])) $_POST[$db] = implode(',', $_POST[$db]);
        if ($_POST[$db]) $_POST[$db] = '0,' . $_POST[$db] . ',0';
        break;
      ...
      case "file":
        if (is_array($_FILES[$db])) $_POST[$db] = $_FILES[$db]['name'];
  ...
  $query .= ")";
 // echo $query;
  $showSPErrorValue = $showSPError;
  $showSPError = 1;
  $bib = my_mysql_query($query);
  $showSPError = $showSPErrorValue;
  $eklenenID = my_mysql_insert_id();
  foreach ($dosyalar as $dosyaID => $uploadto) {
    ...
    if (strpos(strtolower($_FILES[$dosyaID]['tmp_name']), '.php') !== false || strpos(strtolower($_FILES[$dosyaID]['tmp_name']), '.asp') !== false || strpos(strtolower($_FILES[$dosyaID]['tmp_name']), '.js') !== false || strpos(strtolower($_FILES[$dosyaID]['tmp_name']), '.hta') !== false)
      ...
      if ($dbase == 'siteConfig' && !in_array($dosyaID, $siteConfigArray) && is_array($siteConfigArray)) {
    ...
    } else $bib = my_mysql_query("update $dbase set $dosyaID = '$dosyaAdi' where $id='$eklenenID'");
