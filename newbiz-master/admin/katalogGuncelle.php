...
$sorgu = $baglanti->prepare("SELECT * FROM katalog Where id=:id");
...
$sonuc = $sorgu->fetch();//sorgu çalıştırılıp veriler alınıyor
       ...
       if ($_POST) { // Post olup olmadığını kontrol ediyoruz.
            ...
            if ($baslik <> "" && $icerik <> "" && $hata == "") { // Veri alanlarının boş olmadığını kontrol ettiriyoruz.
             ...
             $sql = "UPDATE katalog SET foto=:foto, baslik=:baslik,aktif=:aktif,sira=:sira, ustBaslik=:ustBaslik, icerik=:icerik WHERE id=:id;";             
             $durum = $baglanti->prepare($sql)->execute($satir);

             if ($durum)
             {
                echo '<script>swal("Başarılı","Katalog güncellendi","success").then((value)=>{ window.location.href = "katalog.php"});

                </script>';     // Eğer güncelleme sorgusu çalıştıysa katalog.php sayfasına yönlendiriyoruz.
            } else {
...
<div id="content-wrapper">
    ...
    <div class="container-fluid">
        ...
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Katalog Ekle</li>
        </ol>
