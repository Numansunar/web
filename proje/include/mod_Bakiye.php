...
function bakiyeTakipGiris() {
	...
	while ($d = my_mysql_fetch_array($q)) {
		...
		$out.='<tr class="'.$class.'">';
		$out.='<td>'.$i.'</td>';
		$out.='<td>'.my_money_format('%i',basketInfo('ToplamKargoDahil',$d['randStr'])).' TL</td>';
	...
	$out.='<td>'.my_money_format('%i',$toplam).' TL</td>';
	$out.='<td></td>';
	$out.='<tr>'."\n";		
...
function bakiye()
{
	...
	$out.='<li class="sf-form-item-fullwidth"><div><label class="sf-text-label">'._lang_mb_kullanilabilirKrediniz."</label> ".my_money_format('%i',$bakiye).' TL</div></li>';	
