...
<head>
	...
	<!-- Invoice Print Style -->
	<link rel="stylesheet" href="../templates/system/admin/template/assets/stylesheets/invoice-print.css" />
...
<body>
	...
	if(is_array($_GET['sipIDs']))
  {
    foreach($_GET['sipIDs'] as $id)
    {
      $filter .= ' OR ID= '.(int)$id.' ';
    }
    $q = mysql_query("select * from siparis where 1=2 $filter");
  }
	while ($siparisData = my_mysql_fetch_array($q)) {
		...
		<page size="A4">
			<div class="invoice">
				...
				<?= str_replace(array('<a ', '</a>'), array('<a_removed ', '</a_removed>'), showBasket(false, $siparisData['randStr'], true)) ?>
			</div>
			<hr />
