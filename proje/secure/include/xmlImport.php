...
{
	$out = "<select id='s_$id' class='catCacheSelect " . ($s ? 'autoload' : '') . "'>";
	$opt .= "<option value='0'>Yeni Kategori Aç</option>";
	...
	if ($s)
		return "<div class='catCacheRow'><div id='s_$id' class='catCacheSelect " . ($s ? 'autoload' : '') . "'>" . ($set ? $set : 'Yeni Kategori Aç') . "</div></div>";
	$out .= $opt;
	$out .= '</select>';
	return $out;
}
...
function fiyatStokKontrolx($urunFiyat, $stok, $urunBirim, $urunCode, $piyasaFiyat = 0, $urunBayiFiyat = 0)
{
	...
	$stopFiyat = false;
	$stopStok = false;
	$urunFiyat = trim($urunFiyat);
	$stok = trim($stok);
	$urunBirim = trim($urunBirim);
	$urunCode = trim($urunCode);
	$piyasaFiyat = trim($piyasaFiyat);
	$urunBayiFiyat = trim($urunBayiFiyat);
	$filter = '';
	...
	if(my_mysql_affected_rows())
	{
		...
		{
			my_mysql_query("update urun $p=1 where tedarikciCode = '$urunCode' AND tedarikciID='$bayiID' limit 1");
		}
	...
	insertTemp('xmlLog', "Kod : $urunCode F: $urunFiyat,S: $stok,B: $urunBirim,PF: $piyasaFiyat,BF: $urunBayiFiyat \n");
}
...
function XMLdownloadFile($file, $local_path, $newfilename)
{
	...
	global $copyFiles, $altDizinAc, $downloadType, $resimMultiCCT;
	$altDizinAc = false;
...
function finishXMLImport()
{
	...
	if (hq("select code from temp where code like 'xmlLog'"))
		my_mysql_query("update temp set text = concat(text,'" . addslashes(iconv('iso-8859-9', 'utf-8', $_SESSION['xmlLog'])) . "') where code like 'xmlLog'");
	else
		my_mysql_query("insert into temp (code,text) values ('xmlLog','" . addslashes(iconv('iso-8859-9', 'utf-8', $_SESSION['xmlLog'])) . "') ");
	...
	if ($_POST['indexKatalog']) {
		...
		echo "Toplam dolu row : " . $toplamDolu . " . Toplam boş row " . $toplamBos . " .\n";
		...
		insertTemp('xmlLog',"Dublicate marka kontrolü yapıldı. Birden fazla kaydı olan $mk marka silindi.\n");
		 */
	}
...
function urunKontrol($kod, $ad, $cat, $resim, $resim2, $resim3, $resim4, $resim5, $marka, $garanti, $vergi, $onDetay, $ozltable, $bayiID, $info = array())
{
	...
	if (!isset($catActive[$ckod])) {
		$pasif = ($ckod && hq("select ID from xmlcatcache where  tedarikciCode like '$ckod' AND (active = 0 OR up=0)") && !hq("select ID from xmlcatcache where  tedarikciCode like '$ckod' AND (active = 1 OR up=1)"));
	...
	if ($_GET['indexResim']) 
	{
		...
		foreach ($iarr as $image) {
			...
			XMLdownloadFile($image, '../images/', $resimLocation);
			$detay = str_replace($image, 'images/' . $resimLocation, $detay);
			$ri++;
		}
	}
...
//	exit();
	...
	insertTemp('xmlLog', "ID : $urunID $ad ($kod) ($resim) bilgileri güncellendi.\n");
	return $urunID;
}
...
{
	global $lastCatXMLCode, $grupKontrol;
	$kod = trim($kod);
	...
	if (!isset($catActive[$kod])) {
		...
		$pasif = ($fx && !hq("select ID from xmlcatcache where tedarikciID = '$bayiID' AND tedarikciCode ='$kod' AND up = 1 AND (active = 1)"));
