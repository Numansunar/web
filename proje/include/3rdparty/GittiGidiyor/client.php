...
class ggClient
{
	...
	public function __construct()
	{
		...
		$this->apiKey = siteConfig('gg_api');
		$this->secretKey = siteConfig('gg_secret');
		$this->nick = siteConfig('gg_username');
		$this->password = siteConfig('gg_password');
		$this->auth_user = siteConfig('gg_username');
		$this->auth_pass = siteConfig('gg_password');
		$this->lang = 'tr';
	...
	public function getSimpleCatalogs($categoryCode, $specs = false)
	{
		$xml = '<categoryCode>' . $categoryCode . '</categoryCode>';
		$options['categoryCode'] = $xml;
		if ($specs) {
			$xml = '<specs>';
			$xml .= $specs;
			$xml .= '</specs>';
		...
		return $this->clientConnect('anonymous', 'CatalogV2', 'getSimpleCatalogs', $options, array('categoryCode', 'specs'));
	}
	...
	public function getCategorySpecs($categoryCode = null)
	{
		...
		$file = $_SERVER['DOCUMENT_ROOT'] . $siteDizini . '/secure/bayiXML/gg_' . $categoryCode . '.txt';
		if (file_exists($file)) {
			$out = unserialize(file_get_contents($file));;
			return $out;
		}

		$out = $this->clientConnect('anonymous', 'Category', 'getCategorySpecs', array($categoryCode));
		dosyayaz($file, serialize($out));
		return $out;
	}
	...
	{
		return $this->updateProductWithNewCargoDetail($itemId, $productId, $product, $onSale = true, $forceToSpecEntry = false, $nextDateOption = false);
	...
	public function getSales(
		...
		return $this->getSalesNew(0, $byStatus, 'A', 'A', 'A', $pageNumber, $pageSize, 'tr');
		return $this->clientConnect('individual', 'Sale', 'getPagedSales', get_defined_vars());
	}
	public function getSales2($withData = true, $byStatus = 'R', $byUser = '', $orderBy = 'A', $orderType = 'A', $pageNumber = 1, $pageSize = 100, $lang = 'tr')
	{
		//$withData = $this->booleanConvert($withData);
	...
	public function getSalesNew(
		...
		//echo debugArray(get_defined_vars());
		$withData = $this->booleanConvert($withData);
		return $this->clientConnect('individual', 'Sale', 'getPagedSales', get_defined_vars());
	}
	...
	protected function clientConnect($serviceAccessType, $serviceType, $method, $parameters, $xml = array())
	{
		...
		switch ($serviceAccessType) {
			...
			case 'individual':
				...
				break;
			case 'internal':
				$url .= 'Internal' . $serviceType . 'Service';
				break;

			case 'community':
				$url .= 'Community' . $serviceType . 'Service';
				break;
		}
		$url .= '?wsdl';
		...
		if ($method == 'getSales') {
			//var_dump($soapParams);exit;
		}
		try {
			$soapClient = new SoapClient($url, array('connection_timeout' => 1000, 'login' => $this->auth_user, 'password' => $this->auth_pass, 'authentication' => SOAP_AUTHENTICATION_BASIC));
			$result = $soapClient->__soapCall($method, $soapParams);
		} catch (Exception $e) {
			return 'GittiGidiyor tarafından bağlantı kesildi. ('.$e.')';
		}
