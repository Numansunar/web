...
if ($_GET['act'] == 'win') 
{
	...
	exit('Promosyon Kodu :<br /><input type="text" class="pro-code" value="' . $code . '" /><br /><div class="basket-button"><a href=\''.slink('sepet').'\'>Sepete Dön</a></div>');
...
function proarckMailGonder($randStr)
{
	...
	if (!my_mysql_num_rows($qTemplate) || hq("select toplamTutarTL from siparis where randStr = '$randStr'") < siteConfig('proCark_min1'))
		return;
...
function myprocark()
{
	...
	if (!siteConfig('proCark_active') && !$_SESSION['admin_isAdmin'])
		return generateTableBox('Hata', 'Promosyon Çarkı Pasif Durumda', tempConfig('formlar'));
	...
	if ((!$procarkIsBlock && ($_GET['op'] != 'info') && !($_GET['op'] == 'demo' && $_SESSION['admin_isAdmin']) && !$shopphp_demo && $check != md5(hq("select ID from siparis where randStr = '" . $randStr . "' && (durum <90)  && ".basketInfo('ModulFarkiIle', $randStr)." >= '" . (float)siteConfig('proCark_min1') . "'").$randStr)) || hq("select ID from promosyon where code = '".$randStr."'"))
	...
	<div id="procark">
	' . ($procarkIsBlock ? '<style>.toast,.cark-tebrik { display:none;}</style> Yapacağınız <strong>' . my_money_format('', siteConfig('proCark_min1')) . ' ' . fiyatBirim('TL') . '</strong> üzeri alışverişten sonra ücretsiz çekilişe katılın. ' : '<button class="spinBtn">ÇEVİR KAZAN!</button>') . '
