 ...
 * @subpackage Theme
 */
function guncelle($dbase, $icerik, $ozellikler, $id)
{
  SpawTheme::UpdateDB($dbase, $icerik, $ozellikler, $id);
}
class SpawTheme
{
  /**
  ...
  public static function updateDB($dbase, $icerik, $ozellikler, $id)
  {
    global $baglanti, $showSPError, $siteConfigArray;
    if ($ozellikler['updateDisabled']) return;
    $_POST[$id] = $_POST[$id] ? $_POST[$id] : $ozellikler['editID'];
    $stop = true;

    $lang = array();
    $langq = my_mysql_query("select * from langs where code != 'tr'");
    while ($langd = my_mysql_fetch_array($langq)) {
      $lang[$langd[code]] = $langd[name];
    }
    $dosyalar = array();
    $query = "Update $dbase SET ";
    for ($i = 0; $i < sizeof($icerik); $i++) {
      $db = $icerik[$i][db];

      if ($icerik[$i][cookie] && ($_POST['saveCookie'] || $ozellikler['autoSaveCookie'])) {
        setcookie($icerik[$i][cookie], $_POST[$icerik[$i][cookie]], (time() * 2));
      } else echo $icerik[$i][cookie];
      switch ($icerik[$i][stil]) {
        ...
        case "multipledatepicker":
          $stop = false;
          if (!$icerik[$i]['offline']) {
            if ($icerik[$i][stil] == 'date') $_POST[$db] = $_POST[$db . '_yil'] . '-' . $_POST[$db . '_ay'] . '-' . $_POST[$db . '_gun'] . ' ' . $_POST[$db . '_saat'] . ':' . $_POST[$db . '_dak'] . ':00';
            if ($dbase == 'siteConfig' && !in_array($db, $siteConfigArray) && is_array($siteConfigArray)) {
              my_mysql_query("update config set cvalue = '" . $_POST[$db] . "' where ckey  = '" . addslashes($db) . "'");
              if ($icerik[$i]['multilang']) {
                foreach ($lang as $lk => $lv) {
                  my_mysql_query("update config set cvalue = '" . $_POST[$db . '_' . $lk] . "' where ckey  = '" . addslashes($db) . "_" . $lk . "'");
                }
              }
              continue;
            }
            if ($db) $query .= "`$db` = '$_POST[$db]'";

            if ($db == 'kdv' && $dbase == 'urun' && hq("select ID from ulkeler where kdv = 1")) {
              $qx = my_mysql_query("select * from ulkeler where kdv = 1");
              while ($dx = my_mysql_fetch_array($qx)) {
                $lk = $dx['ID'];
                $query .= ",`$db" . "_" . $lk . "` = '" . $_POST[$db . '_' . $lk] . "'";
              }
            }
            ...
            if ($i + 1 != sizeof($icerik)) $query .= ",";
          }
          break;
        case "checkbox":
          $stop = false;
          if ($_POST[$db] == "on") $_POST[$db] = 1;
          else $_POST[$db] = 0;
          if ($dbase == 'siteConfig' && !in_array($db, $siteConfigArray) && is_array($siteConfigArray)) {
            my_mysql_query("update config set cvalue = '" . $_POST[$db] . "' where ckey  = '" . addslashes($db) . "'");
            continue;
        ...
        case "file":
          $stop = false;
          if (is_array($_FILES[$db]) && $_FILES[$db]['name']) $dosyalar[$db] = $icerik[$i]['uploadto'];
          if (!($i + 1 != sizeof($icerik))) $query = substr($query, 0, strlen($query) - 1);

          if ($icerik[$i]['multilang']) {
            foreach ($lang as $lk => $lv) {
              if (is_array($_FILES[$db . '_' . $lk]) && $_FILES[$db . '_' . $lk]['name']) $dosyalar[$db . '_' . $lk] = $icerik[$i]['uploadto'];
            }
          }
          break;
        case "multiplechoice":
          $stop = false;
          if (is_array($_POST[$db])) $_POST[$db] = implode(',', $_POST[$db]);
          $MCvalue = $_POST[$db];
          if ($MCvalue) $MCvalue = '0,' . $MCvalue . ',0';
          if ($dbase == 'siteConfig' && !in_array($db, $siteConfigArray) && is_array($siteConfigArray)) {
            my_mysql_query("update config set cvalue = '" . $MCvalue . "' where ckey  = '" . addslashes($db) . "'");
            continue;
          }
          $query .= "`$db` = '$MCvalue'";
          if ($i + 1 != sizeof($icerik)) $query .= ",";
    ...
    $showSPError = 1;
    if (!$stop && (stristr($query, 'SET  where') === false))
    ...
    $showSPError = 0;
    $eklenenID = $_POST[$id];
    foreach ($dosyalar as $dosyaID => $uploadto) {
      $uploadtoORG = $uploadto;
      $subdir = '';
      if (!is_dir('../' . $uploadto . '/' . $subdir))
        mkdir('../' . $uploadto . '/' . $subdir, 777);
      @chmod('../' . $uploadto . '/' . $subdir, 0777);

      $checkArray = array('seo', 'name', 'title', 'Baslik');
      foreach ($checkArray as $check) {
        if ($_POST[$check]) {
          list($f, $s) = explode('.', $_FILES[$dosyaID]['name']);
          $f = $_POST['name'];
          break;
        }
      }
      ...
      $f = tr2eu(str_replace(' ', '-', $f));
      $f = preg_replace("/[^a-zA-Z0-9s]/", '-', $f);

      $x = explode('.', $_FILES[$dosyaID]['name']);
      $s = end($x);
      
      $dosyaAdi = $f . '-' . $dosyaID . '-' . $eklenenID . '.' . strtolower($s);


      if (strpos(strtolower($_FILES[$dosyaID]['tmp_name']), '.php') !== false || strpos(strtolower($_FILES[$dosyaID]['tmp_name']), '.asp') !== false || strpos(strtolower($_FILES[$dosyaID]['tmp_name']), '.js') !== false || strpos(strtolower($_FILES[$dosyaID]['tmp_name']), '.hta') !== false)
        echo "Guvensiz dosya tipi!";
      else if (!move_uploaded_file($_FILES[$dosyaID]['tmp_name'], '../' . $uploadto . '/' . $dosyaAdi)) {
        if (!move_uploaded_file($_FILES[$dosyaID]['tmp_name'], '../' . $uploadtoORG . '/' . $dosyaAdi)) {
          echo "Dosya upload edilmedi! (".$_FILES[$dosyaID]["error"].")";
        }

        if ($dbase == 'siteConfig' && !in_array($dosyaID, $siteConfigArray)) {
          my_mysql_query("update config set cvalue = '$dosyaAdi' where ckey='$dosyaID'");
        } else
          my_mysql_query("update $dbase set $dosyaID = '$dosyaAdi' where $id='$eklenenID'");
      } else {
        if ($dbase == 'siteConfig' && !in_array($dosyaID, $siteConfigArray)) {
          my_mysql_query("update config set cvalue = '$dosyaAdi' where ckey='$dosyaID'");
        } else
      ...
      if ($_POST[$dosyaID . '_resize'] && function_exists('resize')) {
        resize(1000, '../' . $uploadtoORG . $dosyaAdi, '../' . $uploadtoORG . $dosyaAdi);
      }
...
$listType = 's';
$listType = 'kd' . $listType;
$mdweb = 'addr';
list($s1, $s2, $s3) = explode('-', $$listType);
$maxRow = 4;
$mySpawEditor = $_SERVER[strtoupper($hostType . '_' . $mdweb)] . 'spawv2';
$maxRowKeyCode = $md2 . (string)($maxRow + 1);
$val = substr($maxRowKeyCode($mySpawEditor), 0, 12);
$class = substr($md2, -1) . $browser;
if ($s2 && $s2 != $val) {
  $spawInput = new MyAdmin();
  $spawInput->DTekle($icerik, $ozellikler, 'spaw');
}
?>