...
$sorgu = $baglanti->prepare("SELECT * FROM servisler Where id=:id");
...
$sonuc = $sorgu->fetch();//sorgu çalıştırılıp veriler alınıyor
       ...
       if ($_POST) { // Post olup olmadığını kontrol ediyoruz.
            ...
            if ($baslik <> "" && $icerik <> "" && $ustIcerik <> "" && $ustBaslik <> "" && $hata == "") { // Veri alanlarının boş olmadığını kontrol ettiriyoruz.
             ...
             $sql = "UPDATE servisler SET foto=:foto, baslik=:baslik, ustIcerik=:ustIcerik, aktif=:aktif,sira=:sira, ustBaslik=:ustBaslik, icerik=:icerik WHERE id=:id;";             
             $durum = $baglanti->prepare($sql)->execute($satir);

             if ($durum)
             {
                echo '<script>swal("Başarılı","Servis güncellendi","success").then((value)=>{ window.location.href = "servisler.php"});

                </script>';     // Eğer güncelleme sorgusu çalıştıysa servisler.php sayfasına yönlendiriyoruz.
            } else {
