...
$sorgu = $baglanti->prepare("SELECT * FROM ortaklar Where id=:id");
...
$sonuc = $sorgu->fetch();//sorgu çalıştırılıp veriler alınıyor
       ...
       if ($_POST) { // Post olup olmadığını kontrol ediyoruz.
            ...
            if ($baslik <> "" && $icerik <> "" && $hata == "") { // Veri alanlarının boş olmadığını kontrol ettiriyoruz.
             ...
             $sql = "UPDATE ortaklar SET foto=:foto, baslik=:baslik,aktif=:aktif,sira=:sira,icerik=:icerik WHERE id=:id;";             
             $durum = $baglanti->prepare($sql)->execute($satir);

             if ($durum)
             {
                echo '<script>swal("Başarılı","Ortak güncellendi","success").then((value)=>{ window.location.href = "ortaklar.php"});

                </script>';     // Eğer güncelleme sorgusu çalıştıysa ortaklar.php sayfasına yönlendiriyoruz.
            } else {
...
<div id="content-wrapper">
    ...
    <div class="container-fluid">
        ...
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Ortak Ekle</li>
        </ol>
